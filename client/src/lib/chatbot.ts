...
export function getBotResponse(message: string): string {
  ...
  if (lowerMessage.includes('próximo jogo') || lowerMessage.includes('próxima partida')) {
    ...
    return 'Olá! Que bom ter você aqui! Como posso ajudar com informações sobre a FURIA CS?';
  }
  else {
    return 'Estou aqui para responder perguntas sobre a FURIA CS! Você pode perguntar sobre próximos jogos, lineup atual, ranking mundial, torneios ou resultados recentes.';
  }
}
...
export const initialMessages: Message[] = [
  {
    id: '1',
    text: 'Olá! Bem-vindo ao chat da FURIA CS! Estou aqui para te fornecer informações sobre o time, jogadores, próximos jogos e tudo relacionado à FURIA no Counter-Strike. O que você gostaria de saber?',
    sender: 'bot',
    time: getCurrentTime()
  }
];
